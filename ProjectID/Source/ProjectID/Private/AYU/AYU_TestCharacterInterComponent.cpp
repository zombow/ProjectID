...
{
	if(holding_prop!=nullptr) // 이미 props를 들고있다면
	{
		UE_LOG(LogTemp, Warning, (TEXT("HoldingItems"))); // log로 item을 들고있다고 출력
		if((!near_puzzles.IsEmpty()) && holding_prop->Tags[1] == near_puzzles[0]->Tags[1]) //가까운 퍼즐이 있고 And 퍼즐과 들고있는 prop이 짝이라면
		{
	...
	else if(!(near_props.IsEmpty())) //near_props가 비어있지않다면 
	{
